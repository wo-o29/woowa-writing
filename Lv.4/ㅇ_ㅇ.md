# 많은 DOM도 부담 없이, 화면에 보이는 것만 렌더링하는 가상 리스트 최적화

> [!NOTE]  
> 가상 리스트는 빠르게 증가하고 보여줘야 하는 대용량 데이터가 많은 경우, 사용자 경험 요구를 충족하기 위한 필수 기술로 자리잡고 있습니다. 수천, 수만 개의 아이템을 한 번에 렌더링하는 것은 과거보다 더 큰 도전이 되었고, 모바일과 저사양 환경에서는 특히 성능 최적화가 사용자 경험에 직결됩니다.
>
> 최신 트렌드에서는 동적 높이, 무한 스크롤, 복잡한 인터랙션을 지원하면서도 가볍고 빠른 렌더링을 구현하는 가상 리스트 기술이 필수로 꼽힙니다.
> 이 글은 가상 리스트의 기본 개념부터 시작해 직접 간단히 구현하는 방법, 주요 라이브러리들의 특징과 비교, 그리고 실제 프로젝트에 적용할 때 고려해야 할 사항까지 체계적으로 설명하는 글입니다.

### 예상 독자

- 10,000개 이상의 대용량 리스트나 무한 스크롤 UI를 개발하며 성능 최적화가 절실한 프론트엔드 개발자
- 가상 리스트를 활용해 동적 높이, 무한 스크롤, 그룹핑 등 복잡한 기능을 구현하고자 하는 분
- React 환경에서 효율적이고 유지보수하기 쉬운 가상 리스트 라이브러리를 찾고 있는 개발자
- 모바일 및 저사양 환경에서 부드러운 사용자 경험을 제공하기 위한 UI 최적화에 관심 있는 분
...
| @tanstack/react-virtual | 유연성이 매우 높고 TanStack 생태계와 높은 호환성            | 직접 렌더링 코드를 작성해야 하며, 비교적 문서가 적은 편   | 수직/수평/양방향 가상 스크롤, 고정 및 동적 아이템 크기, 무한 스크롤, container/window 스크롤 | 유연함                      |
...
> [!IMPORTANT]
> 전체적으로 성능이 개선되었지만 가상 리스트를 적용하는 추가적인 스크립트로 인해 스크립트 실행 시간이 증가하고, 스크롤 위치를 계산하는 로직, 아이템 동적 교체 등으로 인해 리렌더링 횟수가 증가한 것을 볼 수 있습니다.
...
가상 리스트는 특히 10,000개 이상의 리스트, 테이블이나 채팅 같은 스크롤이 많은 UI, 모바일과 저사양 환경에서 필수적입니다. 동적 높이와 무한 스크롤 같은 복잡한 기능을 효과적으로 지원하며, 라이브러리를 활용하면 구현 효율성과 유지보수성을 높일 수 있습니다.
